...
require(['/boxuegu/bxg/assets/js/config.js'],function () {
    //第一个require函数执行后,就会加载config.js,config.js加载后,别名就配置完成 了
    //就会执行这里的回调函数
    //我们在这个函数里又调用一个require函数,加载其他的js
    require(['jquery','template','bootstrap','/boxuegu/bxg/assets/js/common.js'],function ($,template) {
        getTeacherList()
        getDetailInfo()
        stopOrStart() //注销启用
